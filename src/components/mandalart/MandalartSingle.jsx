...
function MandalartSingle({jsonData, openModal}) {
    ...
    let mandalatSingleArr =  Array.from(Array(3).fill({}), () => new Array(3).fill({})); // 3 x 3 빈배열 생성
    ...
    const renderTable = () => {
        ...
        <tr key={`row-${rowIndex}`}>
            {valueArr.map((value, colIndex) => {
                ...
                >
                    <button style={{ cursor : location.pathname === modalOpenPath ? 'pointer' : 'default' }} onClick={() => handleClick(value)}
